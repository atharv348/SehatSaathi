...
import { 
  LogOut, Home, Activity, Heart, TrendingUp, 
  MessageCircle, User, Menu, X, Stethoscope, 
  Apple, BarChart3 
} from 'lucide-react';
...
function Layout() {
  ...
  const menuItems = [
    ...
    { path: '/vitals', label: 'Vital Signs', icon: Heart },
    { path: '/records', label: 'Health Records', icon: BarChart3 },
    { path: '/workout', label: 'Workout Plan', icon: Activity },
    { path: '/meals', label: 'Meal Plan', icon: Apple },
    { path: '/progress', label: 'Progress', icon: TrendingUp },
    { path: '/coach', label: 'AI Coach AROMI', icon: MessageCircle },
    { path: '/profile', label: 'Profile', icon: User },
  ];

  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-cyan-50 to-teal-50 flex flex-col md:flex-row">
      {/* Mobile Header */}
      ...
      `}>
        <div className="p-6 flex flex-col h-full">
              ...
              return (
                <li key={item.path}>
                  <Link
                    ...
                    className={`flex items-center p-3.5 rounded-xl transition-all group relative overflow-hidden ${
                      isActive(item.path)
                        ? 'bg-gradient-to-r from-blue-600 to-cyan-600 text-white shadow-md border-2 border-blue-300'
                        : 'text-blue-700 hover:bg-blue-50 hover:text-blue-900 border-2 border-transparent hover:border-blue-200'
                    }`}
                  >
                    {/* Active Indicator */}
                    {isActive(item.path) && (
                      <div className="absolute left-0 top-0 bottom-0 w-1.5 bg-cyan-300 rounded-r-full"></div>
                    )}
          ...
          >
            <LogOut className="mr-3 group-hover:scale-110 transition-transform" size={20} />
